...
Example script for training a AW-Opt agent by manually setting portions of the config in
python code.

To see a quick training run, use the following command:

    python train_awopt.py --debug

To run a full length training run on your own dataset, use the following command:

    python train_awopt.py --dataset /path/to/dataset.hdf5 --output /path/to/output_dir
"""
import argparse
...
def get_config(dataset_type="robosuite", dataset_path=None, output_dir=None, debug=False):
    ...
    config = config_factory(algo_name="awopt")

    ### Experiment Config ###
    config.experiment.name = f"{dataset_type}_awopt_example"  # name of experiment used to make log files
