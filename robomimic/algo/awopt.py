"""
Implementation of AW-Opt.
(Paper - https://arxiv.org/abs/2111.05424).

0. CEM (Cross Entropy Method)

1. AWAC (Advantage Weighted Actor Critic, https://arxiv.org/abs/2006.09359)

================= Positive Sample Filtering ======================
2. AWAC_P (AWAC + Positive Filtering)

================= Hybrid Actor-Critic Exploration ======================
3. AWAC_P_ELRS (AWAC + Positive Filtering + Episode Level Random Switcher)

================= Action Selection in the Bellman Update ======================
4. AWAC_P_ELRS_CEM (AWAC_P_ELRS + CEM)
5. AWAC_P_ELRS_PI_MEAN (AWAC_P_ELRS + CEM + initial mean from the actor pi)
6. AWAC_P_ELRS_PI_CAND (AWAC_P_ELRS + CEM + combines the actor predicted action as an additional candidates)
...
@register_algo_factory_func("awopt")
def algo_config_to_class(algo_config):
    """
    Maps algo config to the AW_Opt algo class to instantiate, along with additional algo kwargs.
    ...
    actor_enabled = ('actor' in algo_config and algo_config.actor.enabled)
    positive_filter_enabled = ("p_filter" in algo_config and algo_config.p_filter.enabled)
    random_switcher_enabled = ("hybrid_actor" in algo_config and algo_config.hybrid_actor.enabled)
    bellman_cem_enabled = ("bellman" in algo_config and algo_config.bellman.enabled)

    if actor_enabled:
        if positive_filter_enabled:
            if random_switcher_enabled:
                if bellman_cem_enabled:
                    if algo_config.bellman.pi_mean:
                        return AWAC_P_ELRS_PI_MEAN, {}
                    elif algo_config.bellman.pi_cand:
                        return AWAC_P_ELRS_PI_CAND, {}
                    return AWAC_P_ELRS_CEM, {}
                return AWAC_P_ELRS, {}
            return AWAC_P, {}
        return AWAC, {}
    else:
    ...
    def _log_critic_info(self, info):
        ...
        for critic_ind in range(len(self.nets["critic"])):
            loss_log["Critic/Critic{}_Loss".format(critic_ind + 1)] = info[
                "critic/critic{}_loss".format(critic_ind + 1)].item()
...
class AWAC_P_ELRS(AWAC_P):
    pass


class AWAC_P_ELRS_CEM(AWAC_P_ELRS):
    pass


class AWAC_P_ELRS_PI_MEAN(AWAC_P_ELRS_CEM):
    pass


class AWAC_P_ELRS_PI_CAND(AWAC_P_ELRS_CEM):
    pass
