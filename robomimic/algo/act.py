...
class ACT(BC_VAE):
    ...
    def _create_networks(self):
        ...
        for k in self.proprio_keys:
            self.proprio_dim += reduce(lambda x, y: x*y, self.obs_key_shapes[k])
        ...
        self.nets["policy"] = torch.nn.parallel.DataParallel(model, device_ids=list(range(0,torch.cuda.device_count())))
    ...
    def process_batch_for_training(self, batch):
        ...
        # For the goal, only keep the camera observations
        if input_batch["goal_obs"] is not None:
            input_batch["goal_obs"]  = {key: value for key, value in batch["goal_obs"].items() if key.startswith('camera')}
    ...
    def _forward_training(self, batch):
        ...
        for cam_name in self.camera_keys:
            ...
            images.append(image)
            goal_image = batch['goal_obs'][cam_name]
            goal_image = self.normalize(goal_image)
            goal_images.append(goal_image)

        images = torch.cat(images, axis=1)
        goal_images = torch.cat(goal_images, axis=1)

        # Concatenate images and goal images channel-wise
        images = torch.cat((images, goal_images), axis=1)
