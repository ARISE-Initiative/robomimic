...
class BCQConfig(BaseConfig):
    ...
    def algo_config(self):
        ...
        self.algo.optim_params.action_sampler.learning_rate.initial = (
            1e-3  # action sampler learning rate
        )
        self.algo.optim_params.action_sampler.learning_rate.decay_factor = (
            0.1  # factor to decay LR by (if epoch schedule non-empty)
        )
        self.algo.optim_params.action_sampler.learning_rate.epoch_schedule = (
            []
        )  # epochs where LR decay occurs
        self.algo.optim_params.action_sampler.regularization.L2 = (
            0.00  # L2 regularization strength
        )
        self.algo.optim_params.action_sampler.start_epoch = (
            -1
        )  # number of epochs before starting action sampler training (-1 means start right away)
        self.algo.optim_params.action_sampler.end_epoch = (
            -1
        )  # number of epochs before ending action sampler training (-1 means start right away)
        ...
        self.algo.critic.num_action_samples_rollout = (
            100  # number of actions to sample per environment step
        )
        ...
        self.algo.critic.ensemble.weight = (
            0.75  # weighting for mixing min and max for target Q value
        )

        # distributional critic
        self.algo.critic.distributional.enabled = (
            False  # train distributional critic (C51)
        )
        self.algo.critic.distributional.num_atoms = (
            51  # number of values in categorical distribution
        )
        ...
        del (
            self.algo.action_sampler.optim_params
        )  # since action sampler optim params specified at top-level
        ...
        self.algo.actor.perturbation_scale = (
            0.05  # size of learned action perturbations
        )
