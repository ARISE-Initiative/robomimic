...
class FrameStackWrapper(EnvWrapper):
    ...
    def _handle_non_np_obs(self, obs):
        """
        Force any observation entries that are non-numpy into numpy.
        """
        return { k : np.array(obs[k]) if not isinstance(obs[k], np.ndarray)  else obs[k] for k in obs }
    ...
    def _get_initial_obs_history(self, init_obs):
        ...
        """
        obs_history = {}
        init_obs = self._handle_non_np_obs(init_obs)
        for k in init_obs:
            obs_history[k] = deque(
    ...
    def reset_obs_history(self, obs):
        """
        Helper function to reset the observation history.
        """
    ...
    def reset(self):
        ...
        """
        obs = self.env.reset()
        return self.reset_obs_history(obs=obs)
    ...
    def get_observation(self):
        """
        Get frame-stacked environment observation.
        """
        obs = self.env.get_observation()
        self.update_obs(obs, reset=False)
        obs = self._handle_non_np_obs(obs)
