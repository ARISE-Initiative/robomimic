...
def env_args_from_teleop_config(teleop_config, env_metadata=None):
    ...
    is_v1_0 = is_v1 and (robosuite.__version__.split(".")[1] == "0")
    ...
    if env_metadata is not None and is_v1:
        ...
        if "bddl_file_name" in env_metadata["robosuite_args"]:
            # support for LIBERO
            robosuite_args["bddl_file_name"] = env_metadata["robosuite_args"]["bddl_file_name"]
...
def teleop_to_env_meta(args):
    ...
    if "teleop_env_metadata" in f["data"].attrs:
        ...
        if "task_name" in teleop_env_config_json:
            # overwrite env name
            env_name = teleop_env_config_json["task_name"]
