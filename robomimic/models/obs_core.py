...
from robomimic.utils.camera_utils import DeFiNeImageCamEncoder, VoltronImageCamEncoder, R3MImageCamEncoder, VC1ImageCamEncoder
from robomimic.utils.camera_utils import ResNetFiLMEncoder
...
class PretrainedVisualCore(EncoderCore, BaseNets.ConvBase):
    """
    ...
    def __init__(
        ...
        Args:
            ...
            feature_dimension (int): if not None, add a Linear layer to
                project output into a desired feature dimension
        """
        super(PretrainedVisualCore, self).__init__(input_shape=input_shape)
        self.flatten = flatten
        ...
        if pool_class is not None:
            ...
            feat_shape = self.pool.output_shape(feat_shape)
            post_proc_list.append(self.pool)
        else:
            self.pool = None
        ...
        if feature_dimension is not None:
            assert self.flatten
            post_proc_list.append(torch.nn.Linear(int(np.prod(feat_shape)), self.feature_dimension))
    ...
    def forward(self, inputs):
        ...
        image = inputs["image"]
        # Sometimes will be language conditioned image representation
        lang_emb = inputs.get("lang_emb", None)

        x = self.nets["backbone"].forward(image=image, lang_emb=lang_emb)
...
class DeFiNeVisualCore(EncoderCore, BaseNets.ConvBase):
    """
    ...
    def __init__(
        ...
        Args:
            ...
            feature_dimension (int): if not None, add a Linear layer to
                project output into a desired feature dimension
        """
        super(DeFiNeVisualCore, self).__init__(input_shape=input_shape)
        self.flatten = flatten
    ...
    def forward(self, inputs):
        ...
        # ]] * image.shape[0]), image.device)

        x = self.nets["backbone"].forward(image=image, intrinsics=intrinsics, extrinsics=extrinsics, lang_emb=lang_emb)
        # x = self.nets["backbone"].forward(inputs["image"])
        x = self.nets["post_proc"](x)
...
class FiLMVisualCore(EncoderCore, BaseNets.ConvBase):
    """
    Extension of VisualCore that takes in camera information
    """
    def __init__(
        ...
        Args:
            ...
            feature_dimension (int): if not None, add a Linear layer to
                project output into a desired feature dimension
        """
        super(FiLMVisualCore, self).__init__(input_shape=input_shape)
        self.flatten = flatten
...
class ColorRandomizer(Randomizer):
    ...
    def _forward_in(self, inputs):
        ...
        ## Special case when image is frame stacked
        ## Apply processsing to each image individually
        if inputs.shape[1] == 6:
            im1 = transform(inputs[:, :3])
            im2 = transform(inputs[:, 3:])
            out = torch.cat([im1, im2], 1)
        else:
