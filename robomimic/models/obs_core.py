...
class VisualCore(EncoderCore, BaseNets.ConvBase):
    ...
    def __init__(
        ...
        # maybe make pool net
        self.pool = None
        self.unsqueeze_1 = None
        self.unsqueeze_2 = None
        if pool_class is not None:
            ...
            # extract only relevant kwargs for this specific backbone
            if len(feat_shape) != 3:
                # hack to allow flat input (like CLIP embedding) to pass through pooling layer like spatial softmax - reshape to [1, D, 1] for [C, H, W]
                assert len(feat_shape) == 1
                self.unsqueeze_1 = Unsqueeze(dim=-1) # adds final dimension
                self.unsqueeze_2 = Unsqueeze(dim=-3) # adds channel dimension
                net_list.append(self.unsqueeze_1)
                net_list.append(self.unsqueeze_2)
                feat_shape = [1, feat_shape[0], 1]
    ...
    def output_shape(self, input_shape):
        ...
        if self.pool is not None:
            # pool output
            if self.unsqueeze_1 is not None:
                feat_shape = self.unsqueeze_2.output_shape(self.unsqueeze_1.output_shape(feat_shape))
            feat_shape = self.pool.output_shape(feat_shape)
...
"""
================================================
MLP Core Networks (thin wrapper around MLP)
================================================
"""
class MLPCore(EncoderCore, BaseNets.ConvBase):
    """
    Thin wrapper around MLP for encoding purposes.
    """
    def __init__(
        self,
        input_shape,
        ...
        Args:
            ...
            feature_dimension (int): if not None, add a Linear layer to
                project output into a desired feature dimension
        """
        super(MLPCore, self).__init__(input_shape=input_shape)
        net_list = [
    ...
    def output_shape(self, input_shape):
        ...
        Returns:
            out_shape ([int]): list of integers corresponding to output shape
        """
        return self.mlp.output_shape(input_shape=input_shape)
    ...
    def forward(self, inputs):
        ...
        ndim = len(self.input_shape)
        assert tuple(inputs.shape)[-ndim:] == tuple(self.input_shape)
        return super(MLPCore, self).forward(inputs)
    ...
    def __repr__(self):
        ...
        indent = ' ' * 2
        msg += textwrap.indent(
            "\ninput_shape={}".format(self.input_shape), indent)
        msg += textwrap.indent("\nMLP={}".format(self.mlp), indent)
        msg = header + '(' + msg + '\n)'
        return msg
